...
Class postController Extends baseController {
    ...
    public function craw(){
        ...
        $this->view->data['title'] = 'Lấy tin tự động';

        $menu = $this->model->get('menuModel');

        $this->view->data['menus'] = $menu->getAllMenu(array('where'=>'menu_id = 2 OR menu_id = 3 OR menu_parent = 2 OR menu_parent = 3'));

        //get link;

        $url ="http://saigonnewport.com.vn/Pages/NewsPage.aspx"; 
        ...
        $content = $this->get_content_by_tag($content,"<div id=\"Left\">",$url); 

    

        $list = $this->list_all_link($content,$url); 

        $label = "";

        $label .= '<optgroup label="TIN TỨC">';

        for ($i=0; $i<count($list); $i++){

            $class = $list[$i]['class']; 

            if($class=="link-folder"){ 
               ...
               if($i==0){ 
                   ...
                   $label .= "<optgroup label=\"".$list[$i]['title']."\">\n"; 
                   ...
                   $label .= "</optgroup><optgroup label=\"".$list[$i]['title']."\">\n";                                
                ...
                $url_sub = 'http://saigonnewport.com.vn/Pages/'.substr($list[$i]['href'],2); 



                $content_sub = $this->get_content_by_url($url_sub); 

            

                $content_sub = $this->get_content_by_tag($content_sub,"<div id=\"Center\">",$url_sub); 

            

                $list_sub = $this->list_all_link($content_sub,$url_sub);



                for ($j=0; $j<count($list_sub); $j++){

                    $class_sub = $list_sub[$j]['class']; 

                    if($class_sub=="link-folder"){ 
                       ...
                       if($j==0){ 
                           ...
                           $label .= "<optgroup label=\"".$list_sub[$j]['title']."\">\n"; 
                           ...
                           $label .= "</optgroup><optgroup label=\"".$list_sub[$j]['title']."\">\n";                                
                    ...
                    }else if($class_sub!='link-subfolder'){ 

                

                        $label .= "<option value=\"".'http://saigonnewport.com.vn/Pages/'.substr($list_sub[$j]['href'],2)."\">-- ".$list_sub[$j]['title']."</option>\n"; 
        ...
        $label .= '<optgroup label="THÔNG BÁO">';



        $url_sub = "http://saigonnewport.com.vn/Pages/ViewBulletin.aspx"; 
                ...
                $content_sub = $this->get_content_by_tag($content_sub,"<div class=\"mainpart\">",$url_sub); 
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
...

            {
                ...
                $noidung = $this->get_content_by_url($url);

    

                    $content = $this->get_content_by_tag($noidung, "<div class=\"divnoidungchinh\">");

                    if(!$content)

                        $content = $this->get_content_by_tag($noidung, "<div class=\"mainpart\">");

                        

                    $title =   $this->get_content_by_tag($noidung, "<h2 class=\"SubHead\">");

                    if(!$title)

                        $title =   $this->get_content_by_tag($noidung, "<b>");

                        

                    $description =   $this->get_content_by_tag($noidung, "<h2 class=\"News_Summary\">");
                ...
                if(strpos($content,'http://saigonnewport.com.vn') == 0){

                    $content = $this->fix_src_img_tag($content, "http://saigonnewport.com.vn");

                    $content = $this->fix_src_link_tag($content, "http://saigonnewport.com.vn");

                }

                echo "{$title}---+++---+++{$description}---+++---+++{$content}---+++---+++$url---+++---+++";
