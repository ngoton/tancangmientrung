...
Class customerController Extends baseController {
    public function index() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $order_by = $this->registry->router->order_by ? $this->registry->router->order_by : 'customer_id';
        ...
        $banks = $bank_model->getAllBank(array('order_by'=>'bank_name','order'=>'ASC'));
        ...
        if ($keyword != '') {
            $search = '( customer_name LIKE "%'.$keyword.'%" OR customer_contact LIKE "%'.$keyword.'%" OR customer_phone LIKE "%'.$keyword.'%" OR customer_type LIKE "%'.$keyword.'%")';
            $data['where'] = $search;
        }
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            $data = array(
                        ...
                        'company_bank' => trim($_POST['company_bank']),
                        'company_bank_number' => trim($_POST['company_bank_number']),
                        'company_bank_branch' => trim($_POST['company_bank_branch']),
                        'company_present' => trim($_POST['company_present']),
    ...
    function export(){
        ...
        $customers = $customer_model->getAllCustomer();
            ...
            $objPHPExcel->setActiveSheetIndex($index_worksheet)
                ...
                ->setCellValue('A3', 'STT')
               ->setCellValue('B3', 'Tên khách hàng')
               ->setCellValue('C3', 'Người liên hệ')
               ->setCellValue('D3', 'Số điện thoại')
               ->setCellValue('E3', 'Loại hàng');
                ...
                foreach ($customers as $row) {
                    ...
                    //$objPHPExcel->setActiveSheetIndex(0)->getStyle('B'.$hang)->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_TEXT );
                     $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('B' . $hang, $row->customer_name)
                        ->setCellValue('C' . $hang, $row->customer_contact)
                        ->setCellValue('D' . $hang, $row->customer_phone)
                        ->setCellValue('E' . $hang, $row->customer_type);
                     $hang++;
            ...
            header("Content-Disposition: attachment; filename= DANH SACH KH.xlsx");
                ...
                for ($row = 2; $row <= $highestRow; ++ $row) {
                    ...
                    if ($val[1] != null ) {
                            ...
                            if(!$customer->getCustomerByWhere(array('customer_name'=>trim($val[1])))) {
                                ...
                                'customer_contact' => trim($val[2]),
                                'customer_phone' => trim($val[3]),
                                'customer_type' => trim($val[4]),
                                );
